...
func (h *AlertConsumerHandler) ConsumeClaim(session sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	for msg := range claim.Messages() {
		...
		if err := json.Unmarshal(msg.Value, &dlqData); err != nil {
			fmt.Printf("⚠️ 收到非 JSON 格式的死信脏数据: %s, 错误: %v\n", string(msg.Value), err)
			// 即使是脏数据也要提交 Offset，否则会一直卡在这里无限消费！
			session.MarkMessage(msg, "")
			continue
		}

		// 2、安全类型断言 (Comma-ok 惯用法)
		jobName, ok1 := dlqData["jobName"].(string)
		errMsg, ok2 := dlqData["errMsg"].(string)
		timeStr, ok3 := dlqData["time"].(string)

		// 赋默认值兜底，防止因为缺失字段导致空打印
		if !ok1 || jobName == "" {
			jobName = "Unknown_Job"
		}
		if !ok2 || errMsg == "" {
		...
		// 3. 模拟触发企业微信/飞书/钉钉的 Webhook
		sendWebhookAlert(jobName, errMsg, timeStr)
